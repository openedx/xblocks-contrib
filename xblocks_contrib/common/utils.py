...
def get_static_resource_path(pipeline_path, debug_path):
    """
    ...
    debug_path (local unpacked asset path).
    ...
    Arguments:
        ...
        debug_path (str): Path used in local debug (REQUIRE_DEBUG=True, pipeline off).

    Returns:
        str: The path to use for local_resource_url().
    """
    pipeline = getattr(settings, 'PIPELINE', {})
    if pipeline.get('PIPELINE_ENABLED', False) or not getattr(settings, 'REQUIRE_DEBUG', False):
