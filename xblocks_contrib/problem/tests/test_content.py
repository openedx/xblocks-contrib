...
from xblocks_contrib.problem.contentstore.content import ContentStore, StaticContent, StaticContentStream
...
class ContentTest(unittest.TestCase):
    ...
    def test_generate_thumbnail_image(self, original_filename, thumbnail_filename):
        ...
        content = Content(AssetLocator(CourseLocator("mitX", "800", "ignore_run"), "asset", original_filename), None)
    ...
    @patch("xblocks_contrib.problem.contentstore.content.Image")
    def test_image_is_closed_when_generating_thumbnail(self, image_class_mock):
        ...
        content_store = ContentStore()
        content = Content(
            AssetLocator(CourseLocator("mitX", "800", "ignore_run"), "asset", "monsters.jpg"), "image/jpeg"
        )
    ...
    def test_store_svg_as_thumbnail(self):
        ...
        content = Content(
            AssetLocator(CourseLocator("mitX", "800", "ignore_run"), "asset", "test.svg"), "image/svg+xml"
        )
    ...
    def test_compute_location(self):
        ...
        assert (
            AssetLocator(
                CourseLocator("mitX", "400", "ignore", deprecated=True), "asset", "subs__1eo_jXvZnE_.srt.sjson"
            )
            == asset_location
        )
    ...
    def test_get_location_from_path(self):
        ...
        assert (
            AssetLocator(
                CourseLocator("a", "b", None, deprecated=True), "asset", "images_course_image.jpg", deprecated=True
            )
            == asset_location
        )
