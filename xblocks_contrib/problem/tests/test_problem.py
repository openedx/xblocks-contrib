...
from unittest.mock import DEFAULT, Mock, PropertyMock, mock_open, patch
...
from xblock.fields import Any, Boolean, Float, Integer, List, ScopeIds, String
...
from xblocks_contrib.problem.fields import Date, RelativeTime, Timedelta
...
def get_test_system(
    ...
    class TestDescriptorSystem(TestRuntime):
        ...
        def handler_url(self, block, handler_name, suffix="", query=""):
            # Return a dummy but valid-looking URL
            return f"http://testserver/xblock/{handler_name}/{suffix}".rstrip("/")
        ...
        def get_field_provenance(self, block, field):
            """A mock provenance method for studio tests."""
            return {
                "default_value": field.default,
                "explicitly_set": field.is_set_on(block),
            }
    ...
    services = {
        ...
        "replace_urls": StubReplaceURLService(),
...
class ProblemBlockTest(unittest.TestCase):
    ...
    def test_submit_problem_correct(self):
        ...
        with patch("xblocks_contrib.problem.capa.correctmap.CorrectMap.is_correct") as mock_is_correct:
            with patch("xblocks_contrib.problem.ProblemBlock.get_problem_html") as mock_html:
    ...
    @patch("xblocks_contrib.problem.ProblemBlock.get_score_with_grading_method")
    @patch("xblocks_contrib.problem.capa.correctmap.CorrectMap.is_correct")
    @patch("xblocks_contrib.problem.ProblemBlock.get_problem_html")
    ...
    def test_submit_problem_grading_method_disable_to_enable(self, mock_html: Mock, mock_is_correct: Mock):
        ...
        with patch(
            "xblocks_contrib.problem.ProblemBlock.is_grading_method_enabled",
    ...
    def test_submit_problem_closed(self):
        ...
        with patch("xblocks_contrib.problem.ProblemBlock.closed") as mock_closed:
    ...
    def test_submit_problem_with_files(self, mock_xqueue_post):
        ...
        #         ),
        #         'xqueue_body': (
        #             '{"student_info": "{\\"anonymous_student_id\\": \\"student\\", '
        #             '\\"submission_time\\": \\"20131117183318\\"}", '
        ...
        #             open(u'/home/ned/edx/edx-platform/common/test/data/uploads/asset.html', 'r'),
        #         path(u'/home/ned/edx/edx-platform/common/test/data/uploads/image.jpg'):
        #             open(u'/home/ned/edx/edx-platform/common/test/data/uploads/image.jpg', 'r'),
        #         path(u'/home/ned/edx/edx-platform/common/test/data/uploads/textbook.pdf'):
        #             open(u'/home/ned/edx/edx-platform/common/test/data/uploads/textbook.pdf', 'r'),
        #     },
        # )
    ...
    @patch("xblocks_contrib.problem.ProblemBlock.get_rescore_with_grading_method")
    ...
    @patch("xblocks_contrib.problem.ProblemBlock.publish_grade")
    def test_rescore_problem_grading_method_disable_to_enable(self, mock_publish_grade: Mock):
        ...
        with patch(
            ...
            with patch(
                "xblocks_contrib.problem.capa.capa_problem.LoncapaProblem.is_grading_method_enabled",
                new_callable=PropertyMock,
                return_value=True,
            ):
    ...
    @patch("xblocks_contrib.problem.ProblemBlock.score_from_lcp")
    ...
    def test_get_score_with_grading_method_calls_grading_method_handler(self):
        ...
        with patch("xblocks_contrib.problem.problem.GradingMethodHandler") as mock_handler:
...
@ddt.ddt
class ProblemBlockEnhancementsTest(unittest.TestCase):
    """
    New tests for ProblemBlock to increase coverage on views, utilities, and XML methods.
    """

    def test_own_metadata(self):
        """Verify `own_metadata` returns only explicitly set settings fields."""
        block = CapaFactory.create(display_name="Explicit Name", max_attempts=5)
        block.data = "<problem>new</problem>"  # content scope
        block.attempts = 1  # user_state scope

        metadata = own_metadata(block)

        self.assertIn("display_name", metadata)
        self.assertEqual(metadata["display_name"], "Explicit Name")
        self.assertIn("max_attempts", metadata)
        self.assertEqual(metadata["max_attempts"], 5)
        self.assertNotIn("data", metadata)
        self.assertNotIn("attempts", metadata)

    def test_handle_fatal_lcp_error(self):
        """Test the fatal error handler for LCP problems."""
        block = CapaFactory.create()

        # With a detailed error
        detailed_html = block.handle_fatal_lcp_error(Exception("Something broke"))
        self.assertIn("Something broke", detailed_html)

        # Without a detailed error
        generic_html = block.handle_fatal_lcp_error(None)
        self.assertIn("Contact course staff", generic_html)
        self.assertNotIn("Something broke", generic_html)

    def test_studio_view(self):
        """Verify the studio_view renders correctly."""
        with patch("xblocks_contrib.problem.problem.render_to_string") as mock_render:
            with patch("xblocks_contrib.problem.problem.resource_loader") as mock_loader:
                mock_loader.load_unicode.return_value = "/* css */"
                mock_render.return_value = "<div>Studio Editor</div>"

                block = CapaFactory.create()
                fragment = block.studio_view({})

                mock_render.assert_called_with("problem.html", unittest.mock.ANY)
                self.assertIn("<div>Studio Editor</div>", fragment.content)
                mock_loader.load_unicode.assert_any_call("static/css/problem_editor.css")
                mock_loader.load_unicode.assert_any_call("static/js/src/edit.js")
                self.assertIn("MarkdownEditingDescriptor", fragment.js_init_fn)

    def test_public_view(self):
        """Test the public view branches."""
        block = CapaFactory.create()

        # Case 1: Runtime supports anonymous state
        block.runtime.suppports_state_for_anonymous_users = True
        with patch.object(block, "student_view", return_value="student_view_content") as mock_student_view:
            content = block.public_view({})
            mock_student_view.assert_called_once()
            self.assertEqual(content, "student_view_content")

        # Case 2: Runtime does NOT support anonymous state
        block.runtime.suppports_state_for_anonymous_users = False
        with patch.object(
            ProblemBlock.__bases__[0], "public_view", create=True, return_value="public_view_fallback"
        ) as mock_super_view:
            content = block.public_view({})
            # Correct the assertion: The mock is called with only the context dictionary.
            mock_super_view.assert_called_once_with({})
            self.assertEqual(content, "public_view_fallback")
    ...
    @ddt.unpack
    def test_pretty_print_seconds(self, seconds, expected_string):
        """Verify `pretty_print_seconds` formats time correctly."""
        block = CapaFactory.create()

        # Mock ngettext to handle pluralization predictably in English
        def mock_ngettext(singular, plural, number):
            return singular if number == 1 else plural

        i18n_service = Mock()
        i18n_service.ngettext = mock_ngettext
        block.runtime.service = Mock(return_value=i18n_service)

        self.assertEqual(block.pretty_print_seconds(seconds), expected_string)

    def test_get_answer(self):
        """Test the `get_answer` AJAX handler."""
        # Case 1: Answer is not available
        block = CapaFactory.create(showanswer="never")
        with self.assertRaises(NotFoundError):
            block.get_answer({})

        # Case 2: Answer is available
        block = CapaFactory.create(showanswer="always")
        mock_answers = {"input_1_2_1": 'The correct answer is <a href="/jump_to_id/foo">here</a>.'}
        with patch.object(block.lcp, "get_question_answers", return_value=mock_answers) as mock_get_answers:
            with patch.object(block.runtime, "publish") as mock_publish:
                # Mock the URL replacer service
                replace_urls_service = Mock()
                replace_urls_service.replace_urls.side_effect = lambda x: x.replace(
                    "/jump_to_id/foo", "/courses/foo/bar"
                )

                def service_side_effect(block_self, service_name):
                    if service_name == "replace_urls":
                        return replace_urls_service
                    # Return a default mock for i18n service
                    return Mock(gettext=lambda s: s)

                # Use patch.object to correctly mock the 'service' method on the runtime instance
                with patch.object(block.runtime, "service", side_effect=service_side_effect):
                    result = block.get_answer({})

                    # Verify event was published
                    mock_publish.assert_called_with(block, "showanswer", {"problem_id": str(block.location)})
                    # Verify answers were fetched from LCP
                    mock_get_answers.assert_called_once()
                    # Verify URL replacer was called
                    replace_urls_service.replace_urls.assert_called_once()
                    # Verify result structure and content
                    self.assertIn("answers", result)
                    self.assertIn("input_1_2_1", result["answers"])
                    self.assertIn("/courses/foo/bar", result["answers"]["input_1_2_1"])
                    self.assertIn("correct_status_html", result)

    def test_max_score(self):
        """Test the `max_score` method."""
        block = CapaFactory.create()

        # Mock LoncapaProblem to control its behavior
        with patch("xblocks_contrib.problem.problem.LoncapaProblem") as mock_lcp:
            # Success case
            mock_lcp.return_value.get_max_score.return_value = 5.0
            self.assertEqual(block.max_score(), 5.0)

            # Error case
            mock_lcp.side_effect = LoncapaProblemError("Bad XML")
            self.assertEqual(block.max_score(), 0)

    def test_add_xml_to_node_export_to_file(self):
        """Test `add_xml_to_node` when exporting to a separate file."""
        block = CapaFactory.create(display_name="My Test Problem", weight=2.0)
        # Add an unknown attribute to be stored in xml_attributes
        block.xml_attributes = {"custom_attr": "custom_val"}
        node = etree.Element("problem")

        # Mock the runtime and filesystem for export
        mock_fs_open = mock_open()
        block.runtime.export_fs = Mock()
        block.runtime.export_fs.open.side_effect = mock_fs_open

        block.add_xml_to_node(node)

        # 1. Verify file was created with the correct path
        filepath = f"problem/{block.url_name}.xml"
        block.runtime.export_fs.makedirs.assert_called_with(os.path.dirname(filepath), recreate=True)
        mock_fs_open.assert_called_with(filepath, "wb")

        # 2. Verify the content written to the file
        handle = mock_fs_open()
        written_content = b"".join(call.args[0] for call in handle.write.mock_calls)
        written_xml = etree.fromstring(written_content)

        self.assertEqual(written_xml.tag, "problem")
        self.assertEqual(written_xml.get("display_name"), "My Test Problem")
        self.assertEqual(written_xml.get("weight"), "2.0")
        self.assertEqual(written_xml.get("custom_attr"), "custom_val")  # Check xml_attributes are exported

        # 3. Verify the original node was updated to be a pointer tag
        self.assertEqual(node.get("url_name"), block.url_name)
        self.assertEqual(len(node), 0)  # Pointer tags have no children

    def test_add_xml_to_node_inline(self):
        """Test `add_xml_to_node` when embedding XML inline."""
        block = CapaFactory.create(display_name="Inline Problem")
        node = etree.Element("problem")

        block.runtime.export_fs = Mock()
        with patch.object(block, "export_to_file", return_value=False):
            block.add_xml_to_node(node)

        # Verify the node itself contains the full definition and attributes
        self.assertEqual(node.tag, "problem")
        self.assertEqual(node.get("display_name"), "Inline Problem")
        self.assertIn(b"<numericalresponse", etree.tostring(node))
