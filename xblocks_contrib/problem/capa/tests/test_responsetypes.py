...
class ResponseTest(unittest.TestCase):
    ...
    def assert_multiple_grade(self, problem, correct_answers, incorrect_answers):
        ...
        This helper method grades each input using the provided problem instance and
        verifies that the correctness matches the expected result.

        Args:
            problem: A capa problem instance containing the response to be graded.
            correct_answers (list): A list of inputs that should be graded as "correct".
            incorrect_answers (list): A list of inputs that should be graded as "incorrect".

        Raises:
            AssertionError: If any input does not match the expected correctness.
        """
...
class MultiChoiceResponseTest(ResponseTest):
    ...
    These tests verify that multiple choice questions are graded correctly, including:
    - Standard grading for correct and incorrect choices
    - Partial credit grading
    - Named choices grading
    - Validation of allowed grading schemes
    - Correct assignment of partial points
    - Contextualized choices using problem variables and scripts
    """
    ...
    def test_contextualized_choices(self):
        ...
        choices = ["$ok0", "$ok1", "$ok2"]
        choice_names = ["$text0 ... (should be $ok0)", "$text1 ... (should be $ok1)", "$text2 ... (should be $ok2)"]
        problem = self.build_problem(script=script, choices=choices, choice_names=choice_names, credit_type="points")
...
class TrueFalseResponseTest(ResponseTest):
    ...
    These tests verify that True/False type questions are graded correctly, including:
    - Grading based on all correct choices being selected
    - Handling named choices
    - Single correct responses
    - Invalid or non-existent choices are marked incorrect
    """
...
class ImageResponseTest(ResponseTest):
    ...
    These tests verify that points submitted by students are correctly graded
    against defined rectangles and regions, including:
    - Single rectangles
    - Multiple rectangles
    - Single and multiple regions
    - Combined rectangles and regions
    - The format of the "show answer" output
    """
    ...
    def test_rectangle_grade(self):
        ...
        correct_inputs = ["[12,19]", "[10,10]", "[20,20]", "[10,15]", "[20,15]", "[15,10]", "[15,20]"]
    ...
    def test_multiple_rectangles_grade(self):
        ...
        incorrect_inputs = ["[4,6]", "[25,15]", "[15,40]", "[15,4]", "[50,55]", "[300, 14]", "[120, 400]"]
        self.assert_multiple_grade(problem, correct_inputs, incorrect_inputs)
...
class SymbolicResponseTest(ResponseTest):
    ...
    These tests verify:
    - Correct grading of single symbolic inputs
    - Handling of complex numbers and matrices
    - Exceptions when multiple inputs are given but only one expected
    - Integration with MathML and Snuggletex server responses
    """
...
class OptionResponseTest(ResponseTest):
    ...
    These tests verify:
    - Correct grading of multiple-choice options
    - Proper handling of options containing quotes
    - Correct evaluation when options are variable-based and depend on script execution
    """
    ...
    def test_variable_options(self):
        ...
        problem = self.build_problem(options=["$a", "$b", "$c"], correct_option="$a", script=script)
...
class FormulaResponseTest(ResponseTest):
    ...
    def test_hint(self):
        ...
        # The expected solution is numerically equivalent to x+2y
        problem = self.build_problem(
            sample_dict=sample_dict, num_samples=10, tolerance=0.01, answer="x+2*y", hints=hints
        )
    ...
    def test_script(self):
        ...
        problem = self.build_problem(
            sample_dict=sample_dict, num_samples=10, tolerance=0.01, answer="$calculated_ans", script=script
        )
    ...
    def test_grade_infinity(self):
        ...
        # Test problem
        problem = self.build_problem(sample_dict=sample_dict, num_samples=10, tolerance="1%", answer="x")
...
class StringResponseTest(ResponseTest):
    ...
    These tests verify:
    - Backward compatibility for `_or_` multiple-answer separators
    - Regular expression grading (case-sensitive/insensitive, special chars, escapes, Unicode)
    - Case-sensitive vs. case-insensitive string matching
    - Handling of bogus escape sequences and empty answers
    - Partial matching and additional answers
    - Hint functionality (static, regexp-based, and computed hints)
    - Randomized hint generation based on problem seed
    """
    ...
    def test_regexp(self):
        ...
        problem = self.build_problem(
            answer="just_sample", case_sensitive=False, regexp=True, additional_answers=regexps
        )
    ...
    def test_case_sensitive(self):
        ...
        # set up problems
        problem_specified = self.build_problem(answer="sample_answer", case_sensitive=True, additional_answers=answers)
        problem_not_specified = self.build_problem(answer="sample_answer", additional_answers=answers)
        problems = [problem_specified, problem_not_specified]
        for problem in problems:
            for answer in answers:
                # Exact string should be correct
    ...
    def test_hints_regexp_and_answer_regexp(self):
        ...
        problem = self.build_problem(
            answer="Michigan", case_sensitive=False, hints=hints, additional_answers=additional_answers, regexp=True
        )
...
class CodeResponseTest(ResponseTest):
    ...
    These tests cover:
    - Queue state handling (`is_queued`, `update_score`, `get_recentmost_queuetime`)
    - Score updating based on external grader messages
    - Conversion of file objects in student answers to filenames
    - Parsing and validation of grader response messages (valid/invalid XML/HTML5)
    - Ensuring correct mapping of results to subproblems
    """
    ...
    def setUp(self):
        ...
        self.problem = self.build_problem(
            initial_display="def square(x):", answer_display="answer", grader_payload=grader_payload, num_responses=2
        )
...
class ChoiceResponseTest(ResponseTest):
    ...
    These tests verify grading behavior for:
    - Radio button (single choice) responses
    - Checkbox (multiple choice) responses with full and partial credit
    - Validation of grading schemes (valid and invalid credit types)
    - Partial credit scoring methods (EDC, halves)
    - Handling of no-selection answers
    - Contextualized dynamic choices generated from scripts
    """
    ...
    def test_checkbox_group_valid_grading_schemes(self):
        ...
        # Change this test if that changes.
        problem = self.build_problem(
            choice_type="checkbox", choices=[False, False, True, True], credit_type="edc,halves,bongo"
        )
        with pytest.raises(LoncapaProblemError):
            input_dict = {"1_2_1": "choice_1"}
            problem.grade_answers(input_dict)
    ...
    def test_checkbox_group_partial_credit_grade(self):
        ...
        problem = self.build_problem(
            choice_type="checkbox", choices=[False, False, True, True, False], credit_type="halves"
        )
    ...
    def test_grade_with_no_checkbox_selected(self):
        ...
        """
        problem = self.build_problem(choice_type="checkbox", choices=[False, False, False])
    ...
    def test_contextualized_choices(self):
        ...
        problem = self.build_problem(script=script, choice_type="checkbox", choices=choices)
...
class NumericalResponseTest(ResponseTest):
    ...
    These tests verify grading behavior for:
    - Exact matching and tolerance-based grading
    - Range-based grading with support for partial credit
    - Additional answers and associated feedback hints
    - Decimal and percentage tolerances
    - Handling of invalid inputs, exceptions, and edge cases
    - Expression-based staff answers
    ...
    def test_additional_answer_grading(self):
        ...
        problem = self.build_problem(
            answer=primary_answer, additional_answers=additional_answers, correcthint=primary_correcthint
        )
    ...
    def test_grade_range_tolerance_partial_credit(self):
        problem_setup = [
            ...
            ["[1.6e-5, 1.9e24)", ["0.000016", "1.6*10^-5", "1.59e24"], ["-1e26", "1.9e26", "1.9*10^26"], ["0", "2e24"]],
    ...
    def test_grade_partial(self):
        ...
        problem = self.build_problem(answer=4, tolerance=0.2, partial_range=3, credit_type="close")
    ...
    @mock.patch("xblocks_contrib.problem.capa.responsetypes.log")
    ...
    def test_grade_infinity(self):
        ...
        with mock.patch("xblocks_contrib.problem.capa.responsetypes.evaluator") as mock_eval:
...
class CustomResponseTest(ResponseTest):
    """
    Unit tests for validating CustomResponse behavior, including inline code,
    function-based grading logic, randomization, exception handling, and
    multi-input scenarios in capa responsetypes.
    """
    ...
    def test_function_code_multiple_input_no_msg(self):
        ...
        problem = self.build_problem(script=script, cfn="check_func", expect="42", num_inputs=2)
    ...
    def test_function_code_with_attempt_number(self):
        ...
        problem = self.build_problem(script=script, cfn="gradeit", expect="42", cfn_extra_args="attempt")
...
class AnnotationResponseTest(ResponseTest):
    """Unit tests for grading logic of AnnotationResponse, ensuring correct, partially correct,
    and incorrect answers are evaluated with proper points."""
...
class ChoiceTextResponseTest(ResponseTest):
    ...
    TEST_PROBLEM_ARGS = {
        ...
        "1_choice_1_input": {"choices": ("true", {"answer": "123", "tolerance": "1"}), "script": ""},
        "1_input_script": {
            "choices": ("true", {"answer": "$computed_response", "tolerance": "1"}),
            "script": "computed_response = math.sqrt(4)",
        },
        "1_choice_2_inputs": {
            "choices": [("true", ({"answer": "123", "tolerance": "1"}, {"answer": "456", "tolerance": "10"}))],
            "script": "",
        },
        ...
        "2_choices_2_inputs": {
            "choices": [("true", {"answer": "123", "tolerance": "0"}), ("false", {"answer": "999", "tolerance": "0"})],
            "script": "",
        },
    }
    ...
    def test_unchecked_input_not_validated(self):
        ...
        self.assert_grade(
            two_choice_two_input, self._make_answer_dict([(True, ["1"]), (False, ["Platypus"])]), "incorrect"
        )
    ...
    def test_interpret_error(self):
        ...
        with self.assertRaisesRegex(StudentInputError, "Could not interpret"):
            ...
            self.assert_grade(
                two_choice_two_input, self._make_answer_dict([(True, ["1"]), (True, ["Platypus"])]), "correct"
            )
    ...
    def test_staff_answer_error(self):
        ...
        with self.assertRaisesRegex(StudentInputError, "The Staff answer could not be interpreted as a number."):
            self.assert_grade(broken_problem, self._make_answer_dict([(True, ["1"]), (True, ["1"])]), "correct")
    ...
    def test_checkbox_grades(self):
        ...
        scenarios = {
            ...
            "2_choices_2_inputs_missing_choice": ("checkbox_2_choices_2_inputs", "incorrect"),
        ...
        checkbox_two_choices = self._make_problem([("true", {}), ("true", {})], "checkboxtextgroup")
