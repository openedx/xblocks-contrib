...
class ResponseXMLFactory(six.with_metaclass(ABCMeta, object)):
    ...
    def create_input_element(self, **kwargs):
        """
        Create and return an etree element representing the CAPA input XML.

        Examples include elements such as <textline />.
        """
        return None

    def build_xml(self, **kwargs):
        """
        Construct and return an XML string for a CAPA response.

        ``kwargs`` is a dictionary that will be passed to
        `create_response_element` and `create_input_element`. See the
        subclasses below for other keyword arguments you can specify.

        For all response types, ``kwargs`` can contain:

        - ``question_text``: The text of the question to display, wrapped in
          ``<label>`` tags.
        - ``explanation_text``: The detailed explanation shown if the user
          answers incorrectly.
        - ``script``: The embedded Python script (a string).
        - ``num_responses``: The number of responses to create
          [DEFAULT: 1].
        - ``num_inputs``: The number of input elements to create
          [DEFAULT: 1].
        - ``credit_type``: A string specifying the partial credit grading
          scheme.
    ...
    def textline_input_xml(**kwargs):
        ...
        - ``math_display``: If True, includes a MathJax display of user input.
    ...
    def choicegroup_input_xml(**kwargs):
        ...
        - ``choices``: List of True/False values indicating whether a
          particular choice is correct. Users must choose all correct options
          to be marked correct. [DEFAULT: [True]]
        - ``choice_names``: List of strings identifying the choices. If
          specified, ensure ``len(choice_names) == len(choices)``.
        - ``points``: List of strings giving partial credit values (0-1) for
          each choice. Interpreted as floats in problem. If specified, ensure
          ``len(points) == len(choices)``.
        """
...
class NumericalResponseXMLFactory(ResponseXMLFactory):
    ...
    def create_response_element(self, **kwargs):
        ...
        - ``answer``: The correct answer (for example, "5").
        - ``correcthint``: The feedback describing the correct answer.
        - ``additional_answers``: A dict of additional answers along with
          their ``correcthint``.
        - ``tolerance``: The tolerance within which a response is considered
          correct. Can be a decimal (for example, "0.01") or a percentage
          (for example, "2%").
        - ``credit_type``: A string specifying the partial credit grading
          scheme.
        - ``partial_range``: The multiplier for the tolerance that still
          provides partial credit in the "close" grading style.
        - ``partial_answers``: A string of comma-separated alternate answers
          that will receive partial credit in the "list" style.
...
class CustomResponseXMLFactory(ResponseXMLFactory):
    ...
    def create_response_element(self, **kwargs):
        ...
        - ``cfn``: The Python code to run. Can be inline code, or the name of
          a function defined in earlier ``<script>`` tags. The callable should
          have the form ``cfn(expect, answer_given, student_answers)`` where
          ``expect`` is a value (see below), ``answer_given`` is a single value
          (for 1 input) or a list of values (for multiple inputs), and
          ``student_answers`` is a dict of answers by input ID.
        - ``expect``: The value passed to the function ``cfn``.
        - ``answer``: Inline script that calculates the answer.
        - ``answer_attr``: The ``answer`` attribute on the tag itself (treated
          as an alias to ``expect``, though ``expect`` takes priority if both
          are given).
        """
        # Retrieve **kwargs
...
class SchematicResponseXMLFactory(ResponseXMLFactory):
    ...
    def create_input_element(self, **kwargs):
        ...
        and ``initial_value``), none of them are used in the CAPA block. For
        testing, this returns a bare-bones version of ``<schematic>``.
        """
...
class CodeResponseXMLFactory(ResponseXMLFactory):
    ...
    def build_xml(self, **kwargs):
        """Build an XML string for a coderesponse."""
        ...
        return super(CodeResponseXMLFactory, self).build_xml(**kwargs)  # pylint: disable=super-with-arguments
...
class FormulaResponseXMLFactory(ResponseXMLFactory):
    ...
    def create_response_element(self, **kwargs):
        ...
        - ``sample_dict``: A dictionary of the form
          ``{ VARIABLE_NAME: (MIN, MAX), .... }`` that specifies the range
          within which to numerically sample each variable to check student
          answers. [REQUIRED]
        ...
        - ``tolerance``: The tolerance within which answers will be accepted
          [DEFAULT: 0.01].
        - ``answer``: The answer to the problem. Can be a formula string or a
          Python variable defined in a script (for example,
          ``"$calculated_answer"`` for a Python variable named
          ``calculated_answer``). [REQUIRED]
...
class ImageResponseXMLFactory(ResponseXMLFactory):
    ...
    def create_input_element(self, **kwargs):
        ...
        - ``rectangle``: String representing the rectangles the user should
          select. Takes the form ``"(x1,y1)-(x2,y2)"``, where the two
          ``(x,y)`` tuples define the corners of the rectangle. Can include
          multiple rectangles separated by a semicolon, for example,
        ...
        REQUIRED: Either ``rectangle`` or ``regions`` (or both).
        """
        # Get the **kwargs
...
class OptionResponseXMLFactory(ResponseXMLFactory):
    ...
    def create_input_element(self, **kwargs):
        ...
        - ``options``: A list of possible options the user can choose from
          [REQUIRED]. You must specify at least two options.
...
class SymbolicResponseXMLFactory(ResponseXMLFactory):
    ...
    def create_response_element(self, **kwargs):
        ...
        (for example, "matrix", "qbit", "imaginary", "numerical").
        """
        # Retrieve **kwargs
...
class ChoiceTextResponseXMLFactory(ResponseXMLFactory):
    ...
    def create_input_element(self, **kwargs):
        ...
        ``[("true", [{"answer": "5", "tolerance": 0}]), ("false", [{"answer": "5", "tolerance": 0}])]``.

        This indicates that the first checkbox/radio is correct and it contains
        a ``<numtolerance_input>`` with an answer of 5 and a tolerance of 0. It
        also indicates that the second has an incorrect radio button or
        checkbox with a ``<numtolerance_input>``.
        """
        choices = kwargs.get("choices", [("true", {})])
        choice_inputs = []
    ...
    def _create_choice_element(self, **kwargs):
        """Create a choice element for a choicetext problem."""
    ...
    def _create_numtolerance_input_element(self, params):
        """Create a ``<numtolerance_input/>`` or ``<decoy_input/>`` element."""
