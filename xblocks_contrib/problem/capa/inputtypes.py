...
from xblocks_contrib.problem.capa import xqueue_interface
from xblocks_contrib.problem.capa.registry import TagRegistry
from xblocks_contrib.problem.capa.util import sanitize_html
...
class ChoiceGroup(InputTypeBase):
    ...
    Use "location" attribute, ie random, top, bottom.
...
class JSInput(InputTypeBase):
    ...
    customresponse. Loads in a sandboxed iframe to help prevent CSS and JS conflicts
    between the frame and the top-level window.
...
class FormulaEquationInput(InputTypeBase):
    ...
    def handle_ajax(self, dispatch, get):  # pylint: disable=arguments-renamed
        """
...
class AnnotationInput(InputTypeBase):
    ...
    (currently ungraded), and then choose from a set of tags/options, which are graded.
...
class ChoiceTextGroup(InputTypeBase):
    ...
    def extract_choices(element, i18n):
        ...
        If we have XML like this (choice names will have been assigned by now)::

            <radiotextgroup>
                <choice correct="true" name="1_2_1_choiceinput_0bc">
                    The number
                        <numtolerance_input
                            name="1_2_1_choiceinput0_numtolerance_input_0"
                            answer="5"/>
        ...
        The function will set up `choices` as follows::
            ...
            choices = [
                ...
                (
                    ...
                    [
                        {
                            'type': 'text',
                            'contents': "False demonstration choice",
                            'tail_text': '',
                            'value': ''
                        }
                    ]
