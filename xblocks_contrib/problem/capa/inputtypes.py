...
class ChemicalEquationInput(InputTypeBase):
    ...
    def preview_chemcalc(self, data):
        """
        Render an HTML preview of a chemical formula or equation.

        `get` should contain a key 'formula' with the value being the formula string.
...
class ChoiceTextGroup(InputTypeBase):
    ...
    **Checkbox Problem**
    ...
    .. note::
        The `<decoy_input/>` tag generates a text input field within an
        incorrect choice. Since the choice itself is incorrect, any value
        entered in the decoy input will also be marked incorrect, so no
        answer needs to be specified for it.
    ...
    def extract_choices(element, i18n):
        ...
        If we have xml as follows (choice names will have been assigned by
        now)::
