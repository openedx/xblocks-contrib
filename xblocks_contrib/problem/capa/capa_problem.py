...
class LoncapaProblem:
    ...
    def _extract_context(self, tree):
        ...
        if all_code:
            ...
            try:
                ...
                msg = Text("Error while executing script code: %s" % err)
    ...
    def _extract_html(self, problemtree):  # pylint: disable=too-many-statements
        """
        ...
        if problemtree.tag in inputtypes.registry.registered_tags():
            ...
            if isinstance(html_result, str):
                try:
                    html_result = etree.fromstring(html_result)
                except etree.XMLSyntaxError:
                    wrapper = etree.Element("span")
                    wrapper.text = html_result
                    html_result = wrapper
