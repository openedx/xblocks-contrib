...
def make_xheader(lms_callback_url, lms_key, queue_name):
    ...
    The Xqueue header is a JSON-serialized dictionary with the following keys:

    - ``lms_callback_url`` (string): URL to which xqueue will return the request.
    - ``lms_key`` (string): Secret key used by the LMS to protect its state.
    - ``queue_name`` (string): Designates a specific queue, e.g., 'MITx-6.00x'.
...
def parse_xreply(xreply):
    ...
    Messages are a JSON-serialized dictionary with the following keys:

    - ``return_code`` (int): 0 for success, 1 for failure.
    - ``content`` (string): A message from xqueue.
