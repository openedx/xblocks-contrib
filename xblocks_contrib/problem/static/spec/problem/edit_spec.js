describe("MarkdownEditingDescriptor", function () {
  ...
  describe("markdownToXml", function () {
    ...
    it("converts numerical response to xml", function () {
      const data =
        MarkdownEditingDescriptor.markdownToXml(`A numerical response problem accepts a line of text input from the student, and evaluates the input for correctness based on its numerical value.
...
<numericalresponse answer="100">
  ...
  <correcthint>main feedback</correcthint>
    ...
    it("converts multiple choice to xml", function () {
      const data =
        MarkdownEditingDescriptor.markdownToXml(`A multiple choice problem presents radio buttons for student input. Students can only select a single option presented. Multiple Choice questions have been the subject of many areas of research due to the early invention and adoption of bubble sheets.
...
<problem>
    ...
    it("converts StringResponse to xml", function () {
      const data =
        MarkdownEditingDescriptor.markdownToXml(`A string response problem accepts a line of text input from the student, and evaluates the input for correctness based on an expected answer within each input box.
...
<problem>
    ...
    it("converts StringResponse with regular expression to xml", function () {
      const data =
        MarkdownEditingDescriptor.markdownToXml(`Who lead the civil right movement in the United States of America?
= | \w*\.?\s*Luther King\s*.*
...
`;
      expect(function () {
        MarkdownEditingDescriptor.markdownToXml(problemContent);
      }).toThrow(new Error(gettext("An answer option has been left blank. Please review and edit the component.")));
