...
<section class="problems-wrapper" data-url="THE_URL">\
  <section class="formulaequationinput">\
    ...
    window.Problem = {};
    window.Problem.inputAjax = jasmine.createSpy("Problem.inputAjax").and.callFake(function () {
    ...
    window.MathJax = { Hub: {} };
    window.MathJax.Hub.getAllJax = jasmine.createSpy("MathJax.Hub.getAllJax").and.returnValue([this.jax]);
    ...
    it("isn't requested for empty input", function (done) {
      ...
      jasmine
        .waitUntil(function () {
          ...
          return window.Problem.inputAjax.calls.count() > 0 || window.MathJax.Hub.Queue.calls.count() > 0;
        })
        .then(function () {
      ...
      jasmine
        ...
        .then(function () {
          jasmine
            .waitUntil(function () {
              return (
                window.Problem.inputAjax.calls.count() > 0 &&
    ...
    it("updates MathJax and loading icon on callback", function (done) {
      ...
      jasmine
        ...
        .then(function () {
          ...
          // Refresh the MathJax.
          expect(window.MathJax.Hub.Queue).toHaveBeenCalledWith(["Text", jax, "THE_FORMULA"]);
        })
        .always(done);
    });
    ...
    it("finds alternatives if MathJax hasn't finished loading", function (done) {
      ...
      jasmine
        ...
        .then(function () {
          ...
          // Refresh the MathJax.
          expect(window.MathJax.Hub.Queue).toHaveBeenCalledWith(["Typeset", jasmine.any(Object), jasmine.any(Element)]);
        })
        .always(done);
    });
      ...
      jasmine
        ...
        .then(function () {
          jasmine
            ...
            .then(function () {
              // Refresh the MathJax.
              expect(window.MathJax.Hub.Queue).toHaveBeenCalledWith(["Text", jax, "\\text{OOPSIE}"]);
              expect($img.css("visibility")).toEqual("hidden");
            })
            .then(done);
        });
  ...
  describe("Multiple callbacks", function () {
    ...
    it("updates requests sequentially", function () {
      ...
      expect(window.MathJax.Hub.Queue).toHaveBeenCalledWith(["Text", this.jax, "THE_FORMULA_0"]);
      expect($img.css("visibility")).toEqual("visible");

      this.callbacks[1](this.responses[1]);
      expect(window.MathJax.Hub.Queue).toHaveBeenCalledWith(["Text", this.jax, "THE_FORMULA_1"]);
      expect($img.css("visibility")).toEqual("hidden");
    });
