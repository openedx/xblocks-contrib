...
formulaEquationPreview.enable = function () {
  ...
  function setupInput() {
    ...
    var throttledRequest = _.throttle(sendRequest, formulaEquationPreview.minDelay, { leading: false });
  ...
  function updatePage(inputData, response) {
    ...
    if (requestStart == inputData.lastSent && !inputData.isWaitingForRequest) {
      // Disable icon.
      inputData.$img.css("visibility", "hidden");
    }
        ...
        if (message === "End") {
          ...
          MathJax.Hub.Queue(function () {
            ...
            if (inputData.jax) {
              ...
              MathJax.Hub.Queue(["Text", inputData.jax, latex]);
            } else if (latex) {
