...
import pymongo
from pymongo.read_preferences import _MODES, _MONGOS_MODES, ReadPreference
...
def connect_to_mongodb(
    db, host, port=27017, tz_aware=True, user=None, password=None, retry_reads=True, **kwargs
    ...
    if "read_preference" in kwargs:
        read_preference = MONGO_READ_PREFERENCE_MAP.get(kwargs["read_preference"], kwargs["read_preference"])
...
def create_collection_index(collection, keys, ignore_created=True, ignore_created_opts=True, **kwargs):
    """
    ...
    try:
        ...
        if exc.code in errors_to_ignore:
            logger.warning(
                "Existing index in collection '%s' remained unchanged!: %s",
