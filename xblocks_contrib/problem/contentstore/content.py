"""
Content management utilities for problem XBlocks.

This module provides the StaticContent class and helpers for
working with asset keys, paths, and canonicalized asset URLs.
"""
...
from xblocks_contrib.problem.assetmgr import AssetManager
from xblocks_contrib.problem.problem import ItemNotFoundError, NotFoundError
...
class StaticContent:
    """
    Represents static assets (files, thumbnails, etc.) in a course.

    Provides helpers to generate asset keys, canonicalized paths,
    and versioned URLs for serving content.
    """
    ...
    def __init__(  # pylint: disable=too-many-positional-arguments
        ...
        self.location = loc
        self.name = (
            name  # a display string which can be edited, and thus not part of the location which needs to be fixed
        )
    ...
    def compute_location(course_key, path, revision=None, is_thumbnail=False):  # pylint: disable=unused-argument
        ...
        return course_key.make_asset_key(
            "asset" if not is_thumbnail else "thumbnail", AssetLocator.clean_keeping_underscores(path)
        ).for_branch(None)
    ...
    def get_static_path_from_location(location):
        """
        This utility static method will take a location identifier and create a 'durable' /static/.. URL
        representation of it. This link is 'durable' as it can maintain integrity across cloning of
        courseware across course-ids, e.g. reruns of courses.

        In the LMS/CMS, we have runtime link-rewriting, so at render time, this /static/... format will get
        translated into the actual /c4x/... path which the client needs to reference static content
        """
    ...
    def get_base_url_path_for_course_assets(course_key):
        """Return the base URL path prefix for course assets."""
...
class StaticContentStream(StaticContent):
    """StaticContent that streams data instead of loading it fully into memory."""
    ...
    def copy_to_in_mem(self):
        """Return a StaticContent copy with data loaded into memory."""
        self._stream.seek(0)
        ...
        try:
            if is_svg:
                ...
                thumbnail_content = StaticContent(
                    thumbnail_file_location, thumbnail_name, "image/svg+xml", thumbnail_file
                )
                ...
                with Image.open(source) as image:
                    ...
                    thumbnail_image.thumbnail(dimensions, Image.LANCZOS)  # pylint: disable=no-member
            ...
            logging.exception(
                "Failed to generate thumbnail for %s. Exception: %s",
                content.location,
                str(exc),
            )
