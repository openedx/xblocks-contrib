...
from xblocks_contrib.problem.contentstore.content import (
    XASSET_LOCATION_TAG,
    ContentStore,
    StaticContent,
    StaticContentStream,
)
from xblocks_contrib.problem.mongo_utils import connect_to_mongodb, create_collection_index
...
class MongoContentStore(ContentStore):
    ...
    def do_connection(self):
        ...
        self.fs = gridfs.GridFS(mongo_db, self.bucket)
    ...
    def save(self, content):
        ...
        thumbnail_location = (
            content.thumbnail_location.to_deprecated_list_repr() if content.thumbnail_location else None
        )
        with self.fs.new_file(
            _id=content_id,
            filename=str(content.location),
    ...
    def find(self, filename, throw_on_not_found=True, as_stream=False):
        content_id, __ = self.asset_db_key(filename)
        ...
        try:
            if as_stream:
                ...
                if thumbnail_location:
                    thumbnail_location = filename.course_key.make_asset_key("thumbnail", thumbnail_location[4])
            ...
            if throw_on_not_found:
                raise NotFoundError(content_id) from exc
            return None

    def export(self, location, output_directory):
        """Export the content at the given location to the specified directory."""
    ...
    def _get_all_content_for_course(  # pylint: disable=too-many-positional-arguments
        self, course_key, get_thumbnails=False, start=0, maxresults=-1, sort=None, filter_params=None
    ):
        """
    ...
    def asset_db_key(cls, location):
        ...
        """
        dbkey = SON(
            (field_name, getattr(location, cls.property_names[field_name])) for field_name in cls.ordered_key_fields
        )
    ...
    def ensure_indexes(self):
        ...
        create_collection_index(
            self.fs_files,
            [("_id.org", pymongo.ASCENDING), ("_id.course", pymongo.ASCENDING), ("displayname", pymongo.ASCENDING)],
            sparse=True,
            background=True,
        )
        create_collection_index(
