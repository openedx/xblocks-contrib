...
from xblocks_contrib.problem.capa.xqueue_interface import XQueueService
from xblocks_contrib.problem.django import contentstore
from xblocks_contrib.problem.fields import Date, ListScoreField, ScoreField, Timedelta
from xblocks_contrib.problem.progress import Progress
...
class ProcessingError(Exception):
    """
    An error occurred while processing a request to the XBlock.
...
class ShowCorrectness:
    ...
    def correctness_available(cls, show_correctness="", due_date=None, has_staff_access=False):
        ...
        if show_correctness == cls.NEVER:
            ...
            return due_date is None or due_date < datetime.datetime.now(datetime.UTC)
...
class ProblemBlock(ScorableXBlockMixin, XBlock):
    """
    ...
    def student_view(self, _context, show_detailed_errors=False):
        ...
        fragment.add_javascript(resource_loader.load_unicode("static/js/src/problem.js"))
    ...
    def studio_view(self, _context):
        ...
        fragment.add_javascript(resource_loader.load_unicode("static/js/src/problem_editor.js"))
        fragment.initialize_js("MarkdownEditingDescriptor")
        return fragment
    ...
    def generate_report_data(self, user_state_iterator, limit_responses=None):
        ...
        Arguments:
            user_state_iterator (iterator): iterator over UserStateClient objects.
        ...
        Returns:
            tuple: Each call returns a tuple like ``("username", {...})``.
    ...
    def handle_fatal_lcp_error(self, error):
        ...
        if error:
            return markupsafe.Markup(
                '<p>Error formatting HTML for problem:</p><p><pre style="color:red">{msg}</pre></p>'
            ).format(msg=str(error))
        else:
            return markupsafe.Markup(
    ...
    def handle_problem_html_error(self, err):
        ...
        if self.debug:
            ...
            msg += markupsafe.Markup("<p>Error:</p><p><pre>{msg}</pre></p>").format(msg=str(err))
            msg += markupsafe.Markup("<p><pre>{tb}</pre></p>").format(tb=traceback.format_exc())
            html = msg
            ...
            _ = self.runtime.service(self, "i18n").gettext
            warning_msg = markupsafe.escape(_("Warning: The problem has been reset to its initial state!"))
            warning = markupsafe.Markup('<div class="capa_reset"> <h2>{}</h2>').format(warning_msg)
            ...
            warning += markupsafe.Markup("{}<ul>").format(warning_msg)

            for student_answer in student_answers.values():
                if student_answer != "":
                    warning += markupsafe.Markup("<li>{}</li>").format(student_answer)

            warning_msg = _("If this error persists, please contact the course staff.")
            warning += markupsafe.Markup("</ul>{}</div>").format(warning_msg)
    ...
    def get_demand_hint(self, hint_index):
        ...
        while counter <= hint_index:
            ...
            total_text = markupsafe.Markup(_("{previous_hints}{list_start_tag}{strong_text}{hint_text}</li>")).format(
                previous_hints=markupsafe.Markup(total_text),
                list_start_tag=markupsafe.Markup('<li class="hint-index-{counter}" tabindex="-1">').format(
                    counter=counter
                ),
                strong_text=markupsafe.Markup("<strong>{hint_number_prefix}</strong>").format(
                    ...
                    hint_number_prefix=markupsafe.escape(_("Hint ({hint_num} of {hints_count}): ")).format(
                        hint_num=counter + 1, hints_count=len(demand_hints)
                    )
                ),
                # Course-authored HTML demand hints are supported.
                hint_text=markupsafe.Markup(
        ...
        total_text = markupsafe.Markup("<ol>{hints}</ol>").format(hints=total_text)
    ...
    def get_problem_html(self, encapsulate=True, submit_notification=False):
        ...
        if encapsulate:
            html = markupsafe.Markup(
                '<div id="problem_{id}" class="problem" data-url="{ajax_url}">{html}</div>'
            ).format(id=self.location.html_id(), ajax_url=self.ajax_url, html=markupsafe.Markup(html))
    ...
    def submit_problem(self, data, override_time=False):  # pylint: disable=too-many-statements
        """
    ...
    def get_submission_metadata_safe(self, answers, correct_map):
        """
        Ensures that no exceptions are thrown while generating input metadata summaries.

        Returns the summary if it is successfully created, otherwise an empty dictionary.
        """
    ...
    def get_submission_metadata(self, answers, correct_map):
        ...
        Returns:
            A dictionary whose keys are a unique identifier for the input (in this case, a capa input_id)
            and whose values are dictionaries containing:

            - question (str)
                The prompt that was presented to the student. Corresponds to the label of the input.
            - answer (mixed)
                The answer the student provided. May be a rich structure, but must be JSON serializable.
            - response_type (str)
                The XML tag of the capa response type.
            - input_type (str)
                The XML tag of the capa input type.
            - correct (bool)
                Whether or not the provided answer is correct. Will be an empty string if correctness
                could not be determined.
            - variant (str)
                In some cases the same question can have multiple variants. This string uniquely
                identifies the variant of the question that was answered. In the capa context, this
                corresponds to the `seed`.
            - group_label (str)
                Optional group label if present in response data.

        This function is conservative and makes very few assumptions about the structure of the
        problem. If problem-related metadata cannot be located, it is replaced with empty strings ''.
        """
